...
> 摘要：记录了 Nginx 的配置文件结构及具体条目的解析，同时例举了实际配置案例，如：宝塔 Linux 面板中 Nginx 1.8 的默认配置、http 反向代理配置案例、https 反向代理配置案例、负载均衡案例、多个 webapp 的配置案例、静态站点配置案例、跨域案例。最后，记录了 upstream 的几种配置方式。
